namespace :manage_adp_can_integration do
  
  desc 'Create ADP CAN integration inventory and configurations'
  task adp_can: :environment do  
    
    puts 'Creating ADP CAN inventory.'
    attributes = { display_name: 'ADP Workforce Now: Canada', status: 2, category: 1, knowledge_base_url: 'https://kallidus.zendesk.com/hc/en-us/articles/360018669837', 
      data_direction: 1, enable_filters: true, api_identifier: 'adp_wfn_can' }
    integration_inventory = IntegrationInventory.where(api_identifier: attributes[:api_identifier]).first_or_create(attributes)
    puts 'Created ADP CAN inventory.'

    puts 'Creating ADP CAN inventory configurations.'
    ...
    attributes = { category: 'settings', field_name: 'Enable Company Code', toggle_context:'Enable multiple company codes', toggle_identifier:'Enable Company Code', width: '100', position: 8}
    integration_inventory.integration_configurations.where("trim(field_name) ILIKE ?", attributes[:field_name]).first_or_create(attributes)
    
    attributes = { category: 'settings', field_name: 'Enable Tax Type', toggle_context:'Enable multiple tax types', toggle_identifier:'Enable Tax Type', width: '100', position: 9}
    integration_inventory.integration_configurations.where("trim(field_name) ILIKE ?", attributes[:field_name]).first_or_create(attributes)
    
    puts 'Created ADP CAN inventory configurations.'
    
    puts 'Uploading ADP CAN inventory logos.'
    display_image_url = "#{Rails.root}/app/assets/images/integration_logos/adp_can.png"
    ...
    puts 'Uploaded ADP CAN inventory logos.'
  end

  desc 'Migrate ADP CAN Credentials'
  task adp_can_credentials: :environment do

    puts 'Creating ADP CAN Instance.'
    Integration.where(api_name: 'adp_wfn_can').find_each do |integration|
    ...
    puts 'Completed ADP CAN Instance creation.'
  end

  desc 'Execute all inventories tasks'
  task all: [:adp_can, :adp_can_credentials]
end