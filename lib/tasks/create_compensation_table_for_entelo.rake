namespace :create_compensation_table_for_entelo do

	desc 'Adding compensation table into custom tables'
	task :add_compensation_table,  [:company_id]=> :environment do |t, args|
		...
		pay_frequency = company.custom_fields.find_by(id: 3275)
		pay_frequency.update(custom_table_id: custom_table.id, position: 1, section: nil)
		rate_type = company.custom_fields.find_by(id: 3276)
		rate_type.update(custom_table_id: custom_table.id, position: 2, section: nil)

    pay_rate1 = company.custom_fields.create!(custom_field_params.merge({name: 'Pay Rate1', field_type: CustomField.field_types[:currency], position: 3}))
	...
	desc 'creating_snapshots_for_compensation_table'
	task :create_custom_table_user_sanpshots_for_compensation_table, [:company_id]=> :environment do |t, args|
		...
		if company.present?
			...
			pay_rate1 = compensation_table.custom_fields.where(name: 'Pay Rate1').first if compensation_table.present?
			effective_date = company.custom_fields.where(name: 'Effective Date', custom_table_id: compensation_table.id).first

			pay_frequency = company.custom_fields.find_by(id: 3275)
			rate_type = company.custom_fields.find_by(id: 3276)
			pay_rate = company.custom_fields.find_by(id: 3277)
