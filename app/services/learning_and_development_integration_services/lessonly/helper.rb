class LearningAndDevelopmentIntegrationServices::Lessonly::Helper
  
  def fetch_integration(company)
    company.integration_instances.find_by(api_identifier: 'lessonly')
  end

  def is_integration_valid?(integration)
    integration.present? && integration.subdomain.present? && integration.api_key.present?
  end
