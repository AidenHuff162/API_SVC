class LearningAndDevelopmentIntegrationServices::Lessonly::UpdateSaplingProfileInLessonly
  ...
  def update
    ...
    begin
      response = execute_update(@integration, request_params, user.lessonly_id)
  ...
  def loggings status, code, parsed_response, request_data, request_params
    create_loggings(@company, 'Lessonly', code, "Update user in less only - #{status}", {response: parsed_response}, {data: request_data, params: request_params}.inspect)
