class LearningAndDevelopmentIntegrationServices::LearnUpon::ParamsBuilder
  ...
  def build_create_profile_params(data)
    { User: build_params(data, 'exclude_in_create'.to_sym, 'parent_hash_path'.to_sym, 'parent_hash'.to_sym) }
  end

  def build_update_profile_params(data)
    { User: build_params(data, 'exclude_in_update'.to_sym, 'parent_hash_path'.to_sym, 'parent_hash'.to_sym) }
  end
