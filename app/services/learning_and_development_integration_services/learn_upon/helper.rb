class LearningAndDevelopmentIntegrationServices::LearnUpon::Helper
  
  def fetch_integration(company)
    company.integration_instances.find_by(api_identifier: 'learn_upon')
  end

  def is_integration_valid?(integration)
    integration.present? && integration.subdomain.present? && integration.username.present? && integration.password.present?
  end
