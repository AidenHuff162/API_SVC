class HrisIntegrationsService::Namely::ParamsBuilder
      ...
      if parameter_mapping.present? 
        if parameter_mapping[:exclude_in_action].blank?
          ...
          if parent_hash_path.blank?
            ...
            temp[key] = value
            parent_params[parameter_mapping[parent_path]] = temp
          end
        end
      elsif key == :user_status || key.to_s.starts_with?("group")
        params[key] = value
      end
    end
