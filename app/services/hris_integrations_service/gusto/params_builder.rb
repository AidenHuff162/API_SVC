class HrisIntegrationsService::Gusto::ParamsBuilder
  ...
  def fetch_value(params, parameter_mapping, key, value, parent_hash)
    if ['user', 'jobs', 'terminate', 'compensations'].include?(parameter_mapping[parent_hash]) 
      { "#{key}": value }
    elsif parameter_mapping[parent_hash] == 'home_address'
      {
        street_1: value[:line1],
        street_2: value[:line2],
        city: value[:city],
        state: State.find_by_name(value[:state])&.key,
        zip: value[:zip],
    ...
    data.each do |key, value|
      ...
      if parameter_mapping[exclude_in_action].blank?
        ...
        if parent_hash_path.present?
          build_hash(params, parent_hash_path.split('|'), fetch_value(params, parameter_mapping, key, value, parent_hash))
        else
          params[key] = value
        end
      end
