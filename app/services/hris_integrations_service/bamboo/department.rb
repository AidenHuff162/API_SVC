class HrisIntegrationsService::Bamboo::Department
  attr_reader :company
  ...
  def fetch
    return [] unless !company.department_mapping_key.blank?

    meta_lists = execute_meta_list_fetch([company.department_mapping_key]).try(:first)['options'] rescue []
