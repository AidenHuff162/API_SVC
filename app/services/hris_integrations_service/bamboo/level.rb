class HrisIntegrationsService::Bamboo::Level
  attr_reader :company
  ...
  def fetch(bamboo_id)
    execute_tabular_data_fetch_custom('customLevel', bamboo_id)
  end

  def create_or_update(action, bamboo_id, params)
    execute_tabular_data_create_or_update_custom(action, 'customLevel', bamboo_id, params)
  end
