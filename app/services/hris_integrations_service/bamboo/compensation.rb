class HrisIntegrationsService::Bamboo::Compensation
  attr_reader :company
  ...
  def fetch(bamboo_id)
    execute_tabular_data_fetch('compensation', bamboo_id)
  end

  def create_or_update(action, bamboo_id, params)
    execute_tabular_data_create_or_update(action, 'compensation', bamboo_id, params)
  end
