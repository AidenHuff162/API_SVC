class PerformanceManagementIntegrationsService::Lattice::ParamsBuilder
  ...
  def build_create_profile_params(data)
    ...
    params.merge!({schemas: ['urn:ietf:params:scim:schemas:core:2.0:User', 'urn:ietf:params:scim:schemas:extension:enterprise:2.0:User', 'urn:ietf:params:scim:schemas:extension:lattice:attributes:1.0:User']})
  end
  ...
  def fetch_value(parameter_mapping, key, value, parent_hash)
    if key.to_s == 'manager' && parameter_mapping[parent_hash] == 'user_extension'
      { value: value }
    elsif ['name', 'lattice_extension', 'user_extension'].include?(parameter_mapping[parent_hash]) 
      { "#{key}": value }
    elsif parameter_mapping[parent_hash] == 'emails' || parameter_mapping[parent_hash] == 'phoneNumbers'
