class PerformanceManagementIntegrationsService::Lattice::Helper
  
  def fetch_integration(company)
    company.integration_instances.find_by(api_identifier: 'lattice')
  end

  def is_integration_valid?(integration)
    integration.present? && integration.api_key.present? && integration.subdomain.present?
  end
