...
class TrackUserChangeReportJob < ApplicationJob
  ...
  def perform(user, report)
    ...
    if report.meta['recipient_type'] == 'sftp' || collection.results.count <= 300
      ...
      file = "Write#{report.report_type.camelize}ReportCSVJob".constantize.set(queue: 'track_user_change_report_job').perform_in(1.second, report.id, collection.results.pluck(:id).uniq, user.id, true)
      [{ inside_csv_job: true }]
    end
  end
end
