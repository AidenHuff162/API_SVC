...
RSpec.describe HrisIntegrations::Paychex::CreatePaychexUserFromSaplingJob , type: :job do
  ...
  it 'should execute service ManageSaplingProfileInPaychex From Sapling Job' do
    result = HrisIntegrations::Paychex::CreatePaychexUserFromSaplingJob.new.perform(user.id)
  ...
  it 'should not execute service ManageSaplingProfileInPaychex From Sapling Job if user not present' do
    result = HrisIntegrations::Paychex::CreatePaychexUserFromSaplingJob.new.perform(nil)
    expect(result).to_not eq('Service Executed')
  end
end
