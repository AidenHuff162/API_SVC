...
RSpec.describe HrisIntegrations::Deputy::TerminateDeputyUserFromSaplingJob , type: :job do
  ...
  it 'should execute service ManageSaplingProfileInDeputy From Sapling Job' do
    result = HrisIntegrations::Deputy::TerminateDeputyUserFromSaplingJob.new.perform(user.id)
  ...
  it 'should not execute service ManageSaplingProfileInDeputy From Sapling Job if user not present' do
    ...
    result = HrisIntegrations::Deputy::TerminateDeputyUserFromSaplingJob.new.perform(nil)
    expect(result).to_not eq('Service Executed')
  end
end
