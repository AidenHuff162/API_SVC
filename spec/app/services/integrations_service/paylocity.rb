...
RSpec.describe IntegrationsService::Paylocity do
  let(:company) { create(:company, subdomain: 'rocketship') }

  subject(:paylocity) { IntegrationsService::Paylocity.new(company) }

  describe '#manage profile setup' do
    it 'manages adp workforce now to paylocity changeover' do
      ...
      status = default_fields.select { |default_field| default_field['name'] == 'Paylocity ID' }.present?
      expect(status).to eq(false)

      paylocity.manage_profile_setup_on_integration_change
    ...
    it 'manages bamboo to paylocity changeover' do
      company.custom_tables.where(custom_table_property: CustomTable.custom_table_properties[:compensation]).destroy_all
      company.integrations.create!(api_name: "bamboo_hr", company_id: company.id)
    ...
    it 'manages namely to paylocity changeover' do
      company.custom_tables.where(custom_table_property: CustomTable.custom_table_properties[:compensation]).destroy_all
    ...
    it 'manages no integration to paylocity changeover' do
      company.custom_tables.where(custom_table_property: CustomTable.custom_table_properties[:compensation]).destroy_all
    ...
    it 'manages adp workforce now and bamboo to paylocity' do
      company.custom_tables.where(custom_table_property: CustomTable.custom_table_properties[:compensation]).destroy_all
      company.integrations.create!(api_name: "ADP-WFN", company_id: company.id)
      company.integrations.create!(api_name: "bamboo_hr", company_id: company.id)
      ...
      expect(custom_fields.count).to eq(1)
      custom_field_options = custom_fields.first.custom_field_options.pluck(:option)
      expect(custom_field_options.count).to eq(9)
