...
feature 'SSO Management', type: :feature, js: true do
  ...
  background { Auth.sign_in_user sarah, sarah.password }
	...
	describe 'Google_SSO' do
		scenario 'Allow sign in with password and Google sso' do
			...
			check_sso_management_after_disabling_google_sso_integration
			sign_out
			check_login_page_for_password_only
		end

		scenario 'Allow sign in with Google_sso only' do
			naviagete_to_integrations
			enable_google_sso
			navigate_to_platform
			navigate_to_sso
			select_sso_only
			sign_out
			check_login_page_for_google_sso_only
			sign_out
			check_login_page_for_password_and_google_sso
			check_email_format
			check_inactive_user
